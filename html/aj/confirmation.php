...
try {
    ...
    if($reservationid) {
        ...
        if($itemid == 0 || $userid == 0)
            throw new Exception("Cannot Reserve non-existent item ($item) by user ($userid)");
    ...
    switch($action) {
        ...
        case 'getsubtotal':
            ...
            $totalcost = $deliveryfee + $totalrent + $tax;
            $due = $totalcost - $deposit;
            ...
            $result = array("success" => true, "tax" => money_format('%i', $tax), "subtotal" => money_format('%i', $totalrent), "deposit" => money_format('%i',$deposit), "deliveryfee" => money_format('%i',$deliveryfee), "totalcost" => money_format('%i',$resobj->getTotal()), "returndate" => $enreturndate, "dueatpickup" => $due, "reservationid" => $reservationid);
