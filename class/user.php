 ...
 *
 *
 *   Use User.php instead of this file for everything now
 *   Use User.php instead of this file for everything now
 *   Use User.php instead of this file for everything now
 *   Use User.php instead of this file for everything now
 *   Use User.php instead of this file for everything now
 ...
 *
*/
// Wow. Sorry. Did this when I was trying to re-learn how to program.
// Code gets progressively better as you move along. Really sorry.
class Users {
    ...
    // OMG This is terrible. Why did I do this?!?!?
	public function __construct( $data = array() ) {
		...
		//	$this->userID = stripslashes( strip_tags( $data['userID'] ) );

		if( isset( $data['code'] ) )
			$this->code = stripslashes( strip_tags( $data['code'] ) );

		if( isset( $data['new_email'] ) )
			$this->new_email = stripslashes( strip_tags( $data['new_email'] ) );
	...
	public function loadFromDB($userID = 0, $username = '',$email = '') {
		...
		try {
			...
			if ($email) {
				$sql =  "SELECT username,first,last,email,password,code,userID ".
			...
			list($this->username, $this->first, $this->last, $this->email,
                             $this->password, $this->code, $this->userID) = $aResult;
